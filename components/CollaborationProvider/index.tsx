...
import { CollaborationContextType, CollaborationProviderProps, Conflict } from './types';
...
export function CollaborationProvider({ children }: CollaborationProviderProps) {
  ...
  const bindMonacoEditor = (editorInstance: MonacoEditor.IStandaloneCodeEditor) => {
    if (yDoc && provider) {
      ...
      yText.observe((event) => {
        event.changes.delta.forEach((change: any) => {
          ...
          const end = start + (change.insert ? change.insert.length : change.delete || 0);
  ...
  const resolveConflict = (index: number, content: string) => {
    if (yDoc) {
      ...
      setConflicts((prevConflicts) => prevConflicts.filter((_, i) => i !== index));
    }
  };
  ...
  const onCursorChange = (event: { position: IPosition }) => {
    if (provider) {
