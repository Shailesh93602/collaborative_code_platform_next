...
export default function PeerList({ roomId, dictionary }: PeerListProps) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-80">
        <h2 className="text-lg font-semibold mb-2">{dictionary?.connectedPeers}</h2>
          ...
          {peers.map((peer) => (
            <div key={peer.id} className="flex items-center space-x-4 mb-2">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{dictionary[peer.status]}</p>
              </div>
            </div>
          ))}
        </ScrollArea>
